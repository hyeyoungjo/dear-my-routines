...
/**
 * Where the → button carries a block (ADR-027). `today` = a fresh plan piece
 * later the same day (not a miss); `tomorrow` / `date` = the classic carry to a
 * later day. The grid maps each to the plan/action matrix.
 */
 ...
 * re-appears as a ghost). The → button opens a destination popover (later today /
 * tomorrow / a picked date, ADR-027) — one control for both plan/action blocks
 * and ghosts, replacing the old context-dependent ✕.
 *
...
export function CalendarBlock({
  ...
  onOpenDetail?: () => void;
  /** Continue this block at a chosen destination (later today / next day). */
  onContinue?: (dest: ContinueDest) => void;
  ...
  // Destination popover for the → button (ADR-027): open? then optionally the
  ...
  // only the action (its plan survives → re-appears as a ghost). Rescheduling
  // (carry/continue) is the → button's job now, not delete (ADR-027).
  const remove = () => {
      ...
      >
        ...
        {isGhost ? (
          ...
          <textarea
            ...
            rows={1}
            // Fill the whole block while editing (h-full stretches to the control
            // row's resolved height) so the click/edit target is the entire block,
            // not just the first line — much easier to hit on a tall block. A
            // title longer than the box scrolls inside the textarea (overflow-auto).
            className="min-h-0 min-w-0 h-full flex-1 resize-none overflow-auto break-words bg-transparent text-xs font-medium leading-tight text-foreground placeholder:font-normal placeholder:text-muted focus:outline-none"
          />
        ) : (
          <div
        ...
        {/* Delete button (🗑) — real blocks only. PLAN drops the plan, ACT drops
            the action. A ghost has no delete: its → carries the plan forward. */}
        {!isGhost && (
          <button
            type="button"
            onClick={(e) => {
            ...
            onPointerDown={(e) => e.stopPropagation()}
            aria-label={kind === "action" ? t("deleteAction") : t("deletePlan")}
            title={kind === "action" ? t("deleteAction") : t("deletePlan")}
            className="shrink-0 text-[10px] text-muted opacity-0 transition-opacity hover:text-red-500 group-hover:opacity-100"
          >
      ...
      {onContinue && (
        <div className="absolute bottom-2 right-1 z-20">
          <button
            ...
            onPointerDown={(e) => e.stopPropagation()}
            aria-label={t("continueLater")}
            title={t("continueLater")}
            className={`rounded text-[10px] transition-opacity ${
              isPartial || continueOpen
