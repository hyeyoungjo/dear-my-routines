...
/**
 * Header link to /guide (usage + what's new). A red dot appears when there's a
 * changelog entry newer than the last time the user opened the guide — that's
 * how new features get surfaced. The dot is client-only (localStorage) and
 * mounted-gated to avoid a hydration mismatch. `/guide` marks itself seen on
 * open, which clears the dot on return. `variant="menu"` is the mobile hamburger row.
 */
