...
5. **AC는 실행 가능한 커맨드** — "~가 동작해야 한다" 같은 추상적 서술이 아닌 실제 실행 가능한 검증 커맨드를 포함한다. 커맨드는 OS 비종속을 위해 `&&`로 한 줄에 이어붙이지 말고 **한 줄에 하나씩** 적는다 (Windows PowerShell 5.1은 `&&`를 파싱하지 못한다).
...
8. **OS 비종속(OS-agnostic)** — 생성되는 코드와 검증 커맨드는 Windows·macOS·Linux에서 모두 동작해야 한다:
   - **파일 입출력은 항상 인코딩을 UTF-8로 명시**한다. 플랫폼 기본 인코딩에 의존하지 마라 — Windows는 cp949/cp1252라 한글·비ASCII가 깨진다.
   - **콘솔(stdout/stderr)에 비ASCII(이모지·기호 등)를 출력**한다면, 파이프·리다이렉트 시 로케일 코덱(Windows cp1252)으로 인코딩돼 크래시할 수 있으니 프로그램 시작 시 스트림을 UTF-8로 재설정한다.
   - **경로 분리자를 하드코딩하지 마라.** 언어가 제공하는 path 유틸(`pathlib`, `path.join` 등)을 쓰고, 문서에는 `/` 슬래시로 적는다.
   - **셸 커맨드를 `&&`로 한 줄에 이어붙이지 마라** (위 5번). 한 줄에 하나씩 적는다.
   - **Python 실행 런처는 OS마다 다르다**: Windows는 `python`, macOS/Linux는 `python3`. step 파일·문서에 둘 다 안내한다.
   - **외부 프로세스에 대용량 입력을 argv로 넘기지 마라.** Windows는 명령줄 길이 한계(WinError 206)가 있으니 stdin/파일로 전달한다.
...
# Windows (python3 별칭이 없으므로 python 사용)
